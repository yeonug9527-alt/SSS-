...
def add_rise_pattern_features(df):
    ...
    start_score=(
        ...
        5*(macd_s>0).astype(float)+
        5*(obv>0).astype(float)
    ).clip(0,100)
...
def apply_cross_sectional_features(universe_data):
    ...
    allr["EntryRaw"]=(
        0.24*entry_rsi.clip(0,100)+
        0.27*entry_disp.clip(0,100)+
        0.16*entry_atr.clip(0,100)+
        0.13*entry_vol.clip(0,100)+
        0.10*allr["PullbackPct"]+
        0.10*entry_breakout
    )
...
st.caption("t일 종가에서만 판단 → 기본 체결은 t+1일 시가. 좋은 종목보다 실제로 상승했던 패턴을 더 깊게 찾아 종목 자체·진입·과거 상승 결과를 분리해 평가합니다.")
